...
class D(nn.Module): # We introduce a class to define the discriminator.
    ...
    def forward(self, input): # We define the forward function that takes as argument an input that will be fed to the neural network, and that will return the output which will be a value between 0 and 1.
        ...
        #.view will flatten into 1 value
...
for epoch in range(25): # We iterate over 25 epochs.
    ...
    #data will be a minibatch, ..........^ i will start from 0
        ...
        #image, label = data ; we ignore the labels of classes from cifar as we just need real/fake.
        ...
        #torch tensors consist of data and gradients too and pytorch accepts only these as input
        target = Variable(torch.ones(input.size()[0])) 
        # We make ground truth vals for each image = 1 as real.
        ...
        # Training the discriminator with a fake images that were generated by the generator
        ...
        # We make ground truths of these fake images as 0

        output = netD(fake.detach()) 
        # output is a torch  tensor with val(prob 0to1) as well as the gradients
        #detach as we donot need the gradients in this case, thus saving memory we detach.
        ...
        optimizerD.step() #applies the optimiser to the network to update weights acc to their respective cntribution
        # We apply the optimizer to update the weights according to how much they are responsible for the loss error of the discriminator.
 		...
 		#Thus, we made the disc a little smart by now.
        


        #-----------------------------------------------------------------------------------
        
        #making the disc think fakes are actually real
        ...
        errG = criterion(output, target)
         ...
         # this error minimized means fake images are close to real.
